...
const SearchablePicker = ({
  ...
  const displayText = useMemo(() => {
    if (multiple) {
      ...
      return currentValues.map((v) => optionsMap.get(String(v)) || v).join(', ');
    } else {
      if (value == null || value === '') return placeholder;
      return optionsMap.get(String(value)) || value;
    }
  ...
  const isSelected = useCallback(
    (optionValue) => {
      if (multiple) {
        return Array.isArray(value) && value.map(String).includes(String(optionValue));
      }
