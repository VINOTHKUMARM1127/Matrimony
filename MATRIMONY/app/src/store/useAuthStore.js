...
const useAuthStore = create((set, get) => ({
  ...
  initialize: async () => {
    try {
      ...
      if (!get().authListenerSubscription) {
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
          ...
          if (get().pendingVerification) {
            console.log('[AuthStore] pendingVerification is true, ignoring auth state change');
            return;
          }
          ...
          if (session) {
            try {
              console.log('[AuthStore] Session found in listener, loading profile...');
              const useProfileStore = require('./useProfileStore').default;
              await useProfileStore.getState().loadProfile(session.user.id);
              console.log('[AuthStore] Profile loaded successfully via listener');
            } catch (e) {
              console.warn('[AuthStore] Profile load failed on auth state change:', e);
            }
            
            console.log('[AuthStore] Setting authenticated state from listener');
            set({
              user: session.user,
              session,
            ...
            console.log('[AuthStore] No session in listener, setting unauthenticated state');
            set({
              user: null,
              session: null,
              isAuthenticated: false,
      ...
      console.log('[AuthStore] Calling authApi.getSession()');
      ...
      console.log('[AuthStore] authApi.getSession() returned, Session exists:', !!session);
