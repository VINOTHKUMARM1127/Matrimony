...
const PREMIUM_PLANS = [
  {
    id: 'gold', // DB key maps to Bronze
    name: 'Bronze Prime',
    duration: '1 Month',
    price: 499,
    currency: 'INR',
    features: [
      'View 40 contact numbers',
      'Unlimited horoscope views',
      'Basic partner matching support',
      'Premium membership badge',
    ],
    color: '#CD7F32',
  },
  {
    id: 'prime_gold', // DB key maps to Silver
    name: 'Silver Prime',
    duration: '1 Month',
    price: 999,
    currency: 'INR',
  ...
  {
    id: 'till_u_marry', // DB key maps to Gold
    name: 'Gold Prime',
    duration: '6 Months',
    price: 2499,
    currency: 'INR',
...
const PremiumScreen = () => {
  ...
  const [selectedPlan, setSelectedPlan] = useState('prime_gold');
  ...
  const calculateExpiry = (planType) => {
    ...
    switch (planType) {
      case 'gold':
        now.setMonth(now.getMonth() + 1); // Bronze = 1 Month
        break;
      case 'prime_gold':
        now.setMonth(now.getMonth() + 1); // Silver = 1 Month
        break;
      case 'till_u_marry':
        now.setMonth(now.getMonth() + 6); // Gold = 6 Months
        break;
      default:
  ...
  const handlePurchase = useCallback(async () => {
    ...
    try {
      if (!RAZORPAY_KEY_ID) {
        // Safe developer fallback if no key is provided
        Alert.alert(
