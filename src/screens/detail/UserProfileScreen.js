...
const UserProfileScreen = ({ route, navigation }) => {
  ...
  const { data: targetProfile, isLoading, error } = useQuery({
  ...
  // 3. Fetch active subscription of viewer
  const { data: activeSub } = useQuery({
    queryKey: ['activeSubscription', currentUser?.id],
    queryFn: async () => {
  ...
  // 4. Fetch phone views count
  const { data: phoneViewsCount = 0, refetch: refetchTotalViews } = useQuery({
    queryKey: ['phoneViewsCount', currentUser?.id],
    queryFn: async () => {
  ...
  // 5. Check if already viewed this target's phone
  ...
  // Privacy Locks / Gates
  const isViewerPremium = myProfile?.is_premium || false;
  const isTargetPremium = targetProfile?.is_premium || false;
  const hasTargetShownInterest = interestStatus && interestStatus.sender_id === profileId;

  const isHoroscopeUnlocked = isViewerPremium || hasTargetShownInterest;
  const isMobileUnlocked = isViewerPremium && isTargetPremium;

  const getPhoneLimitInfo = () => {
    if (!activeSub) return { limit: 0, current: 0 };
    if (activeSub.plan_type === 'gold') return { limit: 40, current: phoneViewsCount };
    if (activeSub.plan_type === 'prime_gold') return { limit: 100, current: phoneViewsCount };
    return { limit: Infinity, current: phoneViewsCount };
  };

  const limitInfo = getPhoneLimitInfo();
  const hasRemainingViews = limitInfo.current < limitInfo.limit;
  ...
  const handleRevealMobileNumber = async () => {
    if (!isMobileUnlocked) {
      Alert.alert(
        'Premium Lock',
        'Mobile numbers can only be shared if BOTH parties have an active premium membership.',
        [
    ...
    if (!hasViewedPhone && !hasRemainingViews) {
      Alert.alert(
        'Limit Exceeded',
        `You have viewed all ${limitInfo.limit} allowed contact numbers.`,
        [
          { text: 'Cancel', style: 'cancel' },
  ...
  const sendInterestMutation = useMutation({
    mutationFn: () => interestApi.sendInterest(currentUser.id, profileId),
    onSuccess: () => {
      refetchInterest();
  ...
  const handleSendInterest = () => {
    const isPremium = useProfileStore.getState().profile?.is_premium;
    if (!isPremium) {
      Alert.alert(
        'Premium Required',
        'Upgrade to Prime to send interests and connect with members directly!',
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Upgrade Now', onPress: () => navigation.navigate('PremiumTab') }
        ]
      );
      return;
    }
  ...
  const acceptInterestMutation = useMutation({
    ...
    onSuccess: () => {
      refetchInterest();
      Alert.alert('Success', 'Interest request accepted! You can now start chatting.');
    },
  });
  ...
  const handleChatPress = async () => {
    try {
      if (!myProfile.is_premium) {
        Alert.alert(
          'Premium Feature',
          'Direct messaging is a Premium feature. Upgrade your subscription to start chatting instantly!',
          [
            { text: 'Cancel', style: 'cancel' },
            { text: 'Upgrade', onPress: () => navigation.navigate('Premium') }
          ]
      ...
      const chat = await chatApi.createChat(currentUser.id, profileId);
      navigation.navigate('Chat', { chatId: chat.id, partnerName: targetProfile.display_name });
    } catch (err) {
          ...
          <View style={styles.contactCard}>
            ...
            {hasViewedPhone ? (
              <View style={styles.revealedRow}>
                <Text style={styles.phoneValue}>{getSimulatedPhoneNumber()}</Text>
              </View>
            ) : (
            ...
            {!hasViewedPhone && isMobileUnlocked && (
              <Text style={styles.limitDisclaimer}>
                Remaining views: {limitInfo.limit - limitInfo.current} / {limitInfo.limit}
              </Text>
            )}
          </View>
        </View>
      ...
      <View style={styles.footer}>
        {interestStatus ? (
          interestStatus.status === 'pending' ? (
            ...
            <View style={styles.footerRow}>
              <TouchableOpacity style={styles.connectBtn} onPress={handleChatPress}>
