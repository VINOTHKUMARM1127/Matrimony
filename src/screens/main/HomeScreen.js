...
const HomeScreen = ({ navigation }) => {
  ...
  const getTierDisplay = () => {
    if (!profile?.is_premium) return 'Free Tier';
    if (!activeSub) return 'Premium Member';
    switch (activeSub.plan_type) {
      case 'gold':
        return 'Prime - Gold Tier';
      case 'prime_gold':
        return 'Prime - Prime Gold Tier';
      case 'till_u_marry':
        return 'Prime - Till U Marry Tier';
      default:
  ...
  const handleRefresh = useCallback(() => {
    refetchRecommended();
    refetchDaily();
  }, [refetchRecommended, refetchDaily]);
  ...
  const renderProfileDashboard = () => (
    <View style={styles.profileDashboard}>
      <LinearGradient
        ...
        style={styles.profileDashboardCard}
      >
        <View style={styles.dashboardAvatarWrapper}>
          ...
          />
          <TouchableOpacity
            style={styles.dashboardUpdatePhotoBtn}
            onPress={() => navigation.navigate('EditProfile')}
          >
            <Text style={styles.dashboardUpdatePhotoIcon}>📷</Text>
          </TouchableOpacity>
        </View>
        ...
        <Text style={styles.dashboardSubtext}>
          Current Package: <Text style={{ fontWeight: '700', color: profile?.is_premium ? colors.gold : colors.textPrimary }}>{getTierDisplay()}</Text>
        </Text>
        <TouchableOpacity
          style={styles.dashboardActionBtn}
          onPress={() => navigation.navigate('EditProfile')}
        >
          <Text style={styles.dashboardActionText}>Update Profile & Photos</Text>
        </TouchableOpacity>
      </LinearGradient>
    </View>
  );
