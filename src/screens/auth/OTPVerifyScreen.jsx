...
const OTPVerifyScreen = ({ route, navigation }) => {
  ...
  const profileData = route.params?.profileData || null; // initial profile data for signup
  const isEmail = !!email;
  ...
  const { verifyEmailOTP, verifyOTP, sendEmailOTP, sendOTP, isLoading, error, clearError } = useAuthStore();
  ...
  const handleVerify = async () => {
    ...
    if (!success) {
      // The store handles setting the global error which syncs to localError
      if (!error) {
    ...
    // Route based on mode
    if (mode === 'signup') {
      // Session is automatically created upon successful OTP verification.
      // Save initial profile data
      if (profileData) {
        try {
          const user = useAuthStore.getState().user;
          if (user) {
            const useProfileStore = require('../../store/useProfileStore').default;
            await useProfileStore.getState().saveProfile({
              id: user.id,
              display_name: profileData.name,
              profile_created_for: profileData.profileFor,
              gender: 'male', // default required field to satisfy db constraints (updated in Step 1)
              date_of_birth: '2000-01-01', // default required field to satisfy db constraints (updated in Step 1)
              mother_tongue: profileData.motherTongue || 'Tamil',
            });
          }
        } catch (err) {
          console.warn('Failed to save initial profile data:', err);
        }
      }
      // AppNavigator handles routing to Registration
    } else if (mode === 'reset') {
      // Navigate to Reset Password screen
      navigation.replace('ResetPassword', { email, phone });
    }
    // mode === 'login' → AppNavigator automatically routes based on isProfileComplete
  };
