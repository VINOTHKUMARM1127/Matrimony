...
const HomeScreen = ({ navigation }) => {
  ...
  const premiumMembers = useMemo(() => {
    if (!recommended) return [];
    return recommended.filter((p) => p.tier && p.tier !== 'free').slice(0, 8);
  }, [recommended]);
            ...
            <View style={styles.quotaRow}>
              <View style={styles.quotaItem}>
                <Text style={styles.quotaValue}>
                  {quotas.contact_credits_remaining === -1 ? '∞' : (quotas.contact_credits_remaining ?? 0)}
                </Text>
              ...
              <View style={styles.quotaItem}>
                <Text style={styles.quotaValue}>
                  {quotas.interest_credits_remaining === -1 ? '∞' : (quotas.interest_credits_remaining ?? 0)}
                </Text>
                <Text style={styles.quotaLabel}>
                  Interests Left
                </Text>
  ...
  const renderPendingInterestsBanner = () => {
    const pendingCount = quotas?.pending_received_interests || 0;
    if (pendingCount <= 0) return null;

    return (
      <TouchableOpacity
        style={[styles.upgradeBanner, { marginTop: 16 }]}
        activeOpacity={0.9}
        onPress={() => navigation.navigate('InterestTab')}
      >
        <LinearGradient
        ...
        >
          ...
          <View style={styles.upgradeContent}>
            <Text style={styles.upgradeTitle}>
              {pendingCount} new {pendingCount === 1 ? 'interest' : 'interests'} received
            </Text>
            <Text style={styles.upgradeSubtext}>
              Check who's interested in your profile
            </Text>
          </View>
