...
const InterestsScreen = ({ navigation }) => {
  ...
  const {
    ...
    queryFn: () => interestApi.getReceivedInterests(user.id, 'all'),
    enabled: !!user?.id,
        ...
        <View style={styles.cardContent}>
          ...
          <View style={styles.cardDetails}>
            <View style={styles.nameRow}>
              ...
              {sender.is_verified && (
                <View style={[styles.verifiedBadge, { marginRight: 'auto', marginLeft: 6 }]}>
                  <Text style={styles.verifiedText}>✓</Text>
                </View>
              )}
              {item.status !== 'sent' && (
                <View style={[styles.statusPill, { backgroundColor: statusBg, marginLeft: 8 }]}>
  ...
  const renderSentItem = ({ item }) => {
    let receiver = item.receiver || item.target;
    if (Array.isArray(receiver)) {
      receiver = receiver[0]; // Handle stale cache from old API format
    }
    ...
    if (item.status === 'accepted') {
      ...
      statusLabel = 'Declined';
      statusBg = colors.errorLight;
    } else if (item.status === 'passed' || activeSubTab === 'passed') {
  ...
  return (
    <SafeAreaView style={styles.safeContainer} edges={['top', 'left', 'right']}>
      ...
      {/* Sub-Filter for Sent/Received */}
      {(activeSubTab === 'received' || activeSubTab === 'sent') && (
        <View style={styles.subFilterContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.subFilterTrack}>
            <TouchableOpacity
