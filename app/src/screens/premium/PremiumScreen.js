...
import { createRazorpayOrder, openCheckout, verifyRazorpayPayment } from '../../services/razorpay';
...
const PremiumScreen = ({ navigation }) => {
  ...
  const handlePurchase = useCallback(async () => {
    ...
    try {
      ...
      let hasRazorpay = Platform.OS === 'web';
      if (!hasRazorpay) {
        try {
          // Try to resolve the module on native.
      ...
      if (!RAZORPAY_KEY_ID || isExpoGo || !hasRazorpay) {
        if (__DEV__ || !hasRazorpay) {
          const simulatePayment = async () => {
            try {
              const { data, error } = await supabase.rpc('fn_simulate_payment', {
                p_plan_id: plan.id
              });
          ...
          if (Platform.OS === 'web') {
            const confirmed = window.confirm(`💳 Matrimony Mock Checkout\n\nSubscribe to ${plan.name} for ₹${plan.price}?`);
            if (confirmed) {
              simulatePayment();
            } else {
              setProcessing(false);
            ...
            // Safe fallback if no key is provided, running inside Expo Go, or missing native module
            Alert.alert(
              '💳 Matrimony Mock Checkout',
              `Expo Go / Developer local sandbox.\n\nSubscribe to ${plan.name} for ₹${plan.price}?`,
              [
                { text: 'Cancel', style: 'cancel', onPress: () => setProcessing(false) },
                { text: 'Simulate Success ✔', onPress: simulatePayment },
              ]
            );
          }
        } else {
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
      ...
      >
        <TouchableOpacity 
          ...
          activeOpacity={0.8}
        >
          <Icon name="chevronLeft" size={28} color="#FFFFFF" strokeWidth={2.5} />
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop: 80, // Space for custom back button
    paddingBottom: 40,
    alignItems: 'center',
    position: 'relative',
